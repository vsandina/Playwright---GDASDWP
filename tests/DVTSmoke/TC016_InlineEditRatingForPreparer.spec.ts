...
test.describe.serial("EditRatingForChecker", () =>
{// Describe the test suite for Edit Rating
    ...
    test("TC012_EditRatingForChecker", async () => {
        // Login
    ...
    // Read buffered requestId
        ...
        const checkboxLabel = page.locator("//label[@for='radc-requests-list-table-checkbox-selected-0']");
        await checkboxLabel.waitFor({ state: 'visible', timeout: 70000 });
        await checkboxLabel.click();
        await page.locator(".dicon.dicon-reviewer-nc").click();
        await page.locator("//label[normalize-space()='Preparer']").click();
        await page.locator('span:nth-child(8) > .star').first().click();
        await page.locator('.dicon.dicon-save-nc').click();
        await page.locator(".load-spinner-tiny").waitFor({ state: 'visible', timeout: 30000 });
        await page.locator("//div[normalize-space()='Saving 1 requests']").waitFor({ state: 'visible', timeout: 30000 });
        await page.waitForTimeout(3000);
        await waitForLoaderToDisappear(page);
        console.log("Edit rating for Preparer completed successfully");
        await page.pause();
});
