...
test.describe.serial("QuickFind", () => {// Describe the test suite for QuickFind
    ...
    test("TC010_QuickFind", async () => {
        ...
        const requestStatusSelector = "div[class='radc-requests-list-virtual-scroller-row ng-star-inserted'] div:nth-child(7)";
        const statusBefore = await page.locator(requestStatusSelector).textContent();
        console.log(statusBefore);

        await page.locator("label[for='radc-requests-list-table-checkbox-selected-0']").click();
        await page.locator(".radc-icon-button.ng-star-inserted").click();
        await page.locator(".ng-select-container.ng-has-value").nth(1).click();
        await page.locator(".ng-option").nth(2).click();
        await page.locator(".dicon.dicon-save-nc.text-primary").click();
        // Wait for status to update after save
        let statusAfter = '';
        let statusUpdateFound = false;
        for (let i = 0; i < 5; i++) {
            await page.waitForTimeout(2000);
            statusAfter = (await page.locator(requestStatusSelector).textContent())?.trim() || '';
            if (statusAfter && statusAfter !== statusBefore) {
                statusUpdateFound = true;
                break;
            }
            await page.locator(".dicon-reload-nc").click();
        }
        if (!statusUpdateFound) {
            throw new Error("Request status did not update after save within expected time.");
        }
