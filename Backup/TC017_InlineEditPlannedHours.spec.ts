...
{// Describe the test suite for QuickFind
    ...
    test("TC015_PlannedHours", async () => {
        ...
        for (let i = 0; i <= 4; i++) {
          await page.getByRole('spinbutton').nth(i).click();
          await page.getByRole('spinbutton').nth(i).fill('8');
        }
        await page.locator('.dicon-save-nc').click(); // Click the Save button
        await page.waitForTimeout(3000); // Wait for the save operation to complete
